...
function RateDataTable({ rates, services, languages, onEdit, onDelete }: RateProps) {
    ...
    return <DataTable value={rates} dataKey="id" scrollable style={{ backgroundColor: "var(--color-white)", width: "100%" }} rowClassName={() => "row-separator"} className="data_table">
        ...
        <Column field="unitPrice" header="Tarif" style={{ minWidth: "150px" }} body={(rowData) => `${rowData.unitPrice.toFixed(4)} \u20ac ${UnitTypes.find(unit => unit.value === rowData.unit)?.name ?? rowData.unit}`} />
